...
const page = () => {
  const [conflicUsername, setConflic] = useState<boolean>(false);
  ...
  const onWriteUsername = async (name: string) => {
    if (name !== "") {
      const responseFetch = await fetch(
        `http://localhost:8080/api/usuarios/getConcidencia/${name}`
      );
      ...
      if (dataResponse.error) {
        Swal.fire({
          icon: "error",
          title: "Error",
          text: "Utilize un nombre de usuario distinto",
        });
      }
    }
  };
      ...
      <div className="panels-container">
        <div className="panel left-panel">
          <div className="content">
            <h3>¿Eres Nuevo?</h3>
            <p>Has clik al siguiente botón para crear tu cuenta !</p>
            <button className="btn transparent" onClick={Open}>
              Regístrate
            </button>
          </div>
          <img src="/images/cafe_3.png" className="image img" alt="" />
        </div>
        <div className="panel right-panel">
          ...
          <img src="/images/cafe_4-.png" className="image" alt="" />
        </div>
      </div>
    </div>
  );
