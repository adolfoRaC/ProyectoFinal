...
const INITIAL_VISIBLE_COLUMNS = ["nombreProducto","direccion","estatus","cantidad" ,"precio"];
  ...
  useEffect(() => {
      const fetchData = async () => {
          if (session?.user.token) {
              try {
                  const response = await axios.get(`http://localhost:8080/api/pedidos/by-cliente/${session.user.id}`, {
                      headers: {
      ...
      case "nombreProducto":
        return (
          ...
          case "telefono":
            ...
            return (
              <div className="flex flex-col">
                <p className="text-bold text-small capitalize">{`${pedido.tienda.direccion.estado.estado} ${pedido.tienda.direccion.municipio.municipio} ${pedido.tienda.direccion.calle} ${pedido.tienda.direccion.colonia} ${pedido.tienda.direccion.CP}  #${pedido.tienda.direccion.numero}`}</p>
              </div>
            ); 
        case "cantidad":
          return (
